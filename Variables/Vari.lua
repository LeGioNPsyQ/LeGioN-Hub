...
pcall(function()
	if isfile and writefile and readfile then
		...
		local function SetWebhook()
			writefile("DevilNetWorkWebhooking.txt", CurrentTime)
			print("[DevilNetWork] Debug: Webhook Delay Set at "..CurrentTime)
			Webhook = GlobalWebhook
		end

		if not isfile("DevilNetWorkWebhooking.txt") then
			SetWebhook()
		elseif tonumber(readfile("DevilNetWorkWebhooking.txt")) < CurrentTime - 7200 then
			SetWebhook()
		else
			Webhook = nil
		end
...
print("[DevilNetWork] Debug: Detected Executor: "..getexploit())
...
task.spawn(function()
	...
	pcall(SendMessage, "[DevilNetWork] Data: DevilNetWork was executed by "..((Player.Name ~= Player.DisplayName and Player.DisplayName) or "Unknown.."..Player.Name:sub(-2, -1)).." on "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." "..VCurrentVersion.." using "..getexploit(), "Execution")
end)
			...
			Callback = function(Value)
				if Value then
					...
					if queueteleport then
						queueteleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/LeGioNPsyQ/LeGioN-Hub/main/LeGioN-Hub.lua"))()')
					end
				end
			end,
		})
					...
					pcall(function()
						if isfile and writefile and readfile then
							...
							local function SetSuggestionsWebhook()
								...
								local success, result = pcall(SendMessage, "[DevilNetWork] Data: "..((Player.Name ~= Player.DisplayName and Player.DisplayName) or "Unknown.."..Player.Name:sub(-2, -1)).." suggested "..Text.." on "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name, "Suggestion")
								if success then
									Notify("Successfully Sent Suggestion", 5)
									writefile("DevilNetWorkWebhooking2.txt", CurrentTime)
									print("[DevilNetWork] Debug: Webhook Delay Set at "..CurrentTime)
								else
							...
							if not isfile("DevilNetWorkWebhooking2.txt") then
								SetSuggestionsWebhook()
							elseif tonumber(readfile("DevilNetWorkWebhooking2.txt")) < CurrentTime - 86400 then
								SetSuggestionsWebhook()
							else
								Webhook = nil
